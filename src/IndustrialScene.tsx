...
function Gear({ radius, teeth, speed, position, color = '#18222c' }: { radius: number; teeth: number; speed: number; position: [number, number, number]; color?: string }) {
  ...
  return <group ref={ref} position={position} rotation={[Math.PI / 2, 0, 0]}>
    <mesh><cylinderGeometry args={[radius * 0.72, radius * 0.72, 0.22, 48]} /><meshStandardMaterial color={color} metalness={0.94} roughness={0.2} /></mesh>
    {Array.from({ length: teeth }).map((_, i) => { const a = (i / teeth) * Math.PI * 2; return <mesh key={i} position={[Math.cos(a) * radius, 0, Math.sin(a) * radius]} rotation={[0, 0, a]}><boxGeometry args={[toothW, 0.24, radius * 0.25]} /><meshStandardMaterial color="#263542" metalness={0.92} roughness={0.22} /></mesh>; })}
    <mesh position={[0, 0.14, 0]}><torusGeometry args={[radius * 0.26, 0.055, 12, 32]} /><meshStandardMaterial color="#f59e0b" emissive="#6b3b00" emissiveIntensity={0.5} metalness={0.88} roughness={0.16} /></mesh>
  </group>;
}
...
function Director() {
  ...
  useFrame(() => {
    ...
    if (hero.current) {
      const f = THREE.MathUtils.smoothstep(p, 0, 0.35);
      hero.current.position.x = THREE.MathUtils.lerp(hero.current.position.x, THREE.MathUtils.lerp(0.2, -3.8, f), 0.035);
    ...
    camera.position.x = THREE.MathUtils.lerp(camera.position.x, input.current.x * 0.3, 0.025);
    camera.position.y = THREE.MathUtils.lerp(camera.position.y, input.current.y * 0.15 + (p - 0.5) * 0.22, 0.025);
  ...
  return <>
    <group ref={hero}><Float speed={1} rotationIntensity={0.12} floatIntensity={0.18}><CNCAssembly /></Float></group>
...
export default function IndustrialScene() {
  return <Canvas dpr={[1, 1.5]} camera={{ position: [0, 0, 9.3], fov: 42 }} gl={{ antialias: true, powerPreference: 'high-performance' }} style={{ position: 'fixed', inset: 0, zIndex: 0, pointerEvents: 'none' }}>
