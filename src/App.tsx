...
function Gear({ radius = 1.3, teeth = 12, y = 0, speed = 0.45 }: { radius?: number; teeth?: number; y?: number; speed?: number }) {
  ...
  return (
    <group ref={ref} position={[0, y, 0]} rotation={[Math.PI / 2, 0, 0]}>
      ...
      {Array.from({ length: teeth }).map((_, i) => (
        <mesh key={i} rotation={[0, 0, (i / teeth) * Math.PI * 2]} position={[Math.cos((i / teeth) * Math.PI * 2) * radius, 0, Math.sin((i / teeth) * Math.PI * 2) * radius]}>
...
function Machine() {
  ...
  return (
    <group ref={group} scale={1.05}>
      ...
      <Gear radius={0.72} teeth={12} y={0.58} speed={0.7} />
      <Gear radius={0.42} teeth={9} y={0.58} speed={-1.05} />
    </group>
  );
}
...
function Scene() {
  return <Canvas dpr={[1, 1.5]} gl={{ antialias: true, powerPreference: 'high-performance' }}>
    ...
    <OrbitControls enableZoom={false} enablePan={false} minPolarAngle={Math.PI / 2.7} maxPolarAngle={Math.PI / 2.05} autoRotate autoRotateSpeed={0.35} />
  </Canvas>;
}
...
export default function App() {
  ...
  return <div className="site">
    <header className="nav"><a className="brand" href="#top"><span className="brand-mark">U</span><span>UJJWAL<span className="muted"> ENGINEERS</span></span></a><button className="menu" onClick={() => setMenu(!menu)}>☰</button><nav className={menu ? 'open' : ''}><a href="#services">Services</a><a href="#about">About</a><a href="#contact">Contact</a><a className="nav-cta" href="mailto:ujjwalelectricals@gmail.com">Start a project ↗</a></nav></header>
    <main id="top">
      <section className="hero"><div className="hero-copy"><div className="eyebrow"><span /> ENGINEERING • FABRICATION • INSTALLATION</div><h1>Engineering that <em>moves</em> industry.</h1><p>Ujjwal Electricals & Mechanical Engineers delivers dependable electrical and mechanical solutions for demanding industrial environments.</p><div className="actions"><a className="primary" href="#contact">Discuss your project <span>→</span></a><a className="secondary" href="#services">Explore capabilities</a></div><div className="stats"><div><strong>10+</strong><span>Years experience</span></div><div><strong>50+</strong><span>Projects delivered</span></div><div><strong>24/7</strong><span>Industrial support</span></div></div></div><div className="scene"><Scene /><div className="scene-label"><span className="pulse" /> LIVE ENGINEERING VIEW<br /><small>PRECISION / PERFORMANCE / RELIABILITY</small></div></div></section>
      <section id="services" className="section"><div className="section-head"><div><span className="kicker">01 / CAPABILITIES</span><h2>Built for the <em>real world.</em></h2></div><p>From electrical systems to mechanical execution, we turn engineering requirements into practical, maintainable industrial solutions.</p></div><div className="cards">{services.map((s, i) => <article className="card" key={s}><span className="num">0{i + 1}</span><div className="icon">{['⌁', '⚙', '◈', '▣'][i]}</div><h3>{s}</h3><p>Precision-led execution with safety, reliability and long-term performance in mind.</p><a href="#contact">Learn more ↗</a></article>)}</div></section>
    ...
    </main><footer><span>© {new Date().getFullYear()} UJJWAL ELECTRICALS & MECHANICAL ENGINEERS</span><span>ENGINEERED FOR PERFORMANCE.</span></footer>
  </div>;
}
